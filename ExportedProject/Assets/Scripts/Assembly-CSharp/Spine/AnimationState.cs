...
namespace Spine
{
	public class AnimationState
	{
		...
		public void Apply(Skeleton skeleton)
		{
			...
			for (int count = tracks.Count; i < count; i++)
			{
				...
				if (num == 1f)
				{
					...
					{
						items2[j].Apply(skeleton, animationLast, animationTime, exposedList, 1f, true, false);
					}
				}
				else
				{
					...
					for (int k = 0; k < count2; k++)
					{
						...
						{
							timeline.Apply(skeleton, animationLast, animationTime, exposedList, num, items4[k], false);
						}
					}
				}
		...
		private float ApplyMixingFrom(TrackEntry entry, Skeleton skeleton)
		{
			...
			bool[] array = ((!multipleMixing) ? mixingFrom.timelinesLast.Items : null);
			float num2 = mixingFrom.alpha * entry.mixAlpha;
			...
			for (int i = 0; i < count; i++)
			{
				...
				bool flag4 = items2[i];
				float alpha = ((array == null || !flag4 || array[i]) ? num3 : num2);
				RotateTimeline rotateTimeline = timeline as RotateTimeline;
				if (rotateTimeline != null)
				{
				...
				{
					timeline.Apply(skeleton, animationLast, animationTime, exposedList, alpha, flag4, true);
				}
			}
			if (entry.mixDuration > 0f)
			{
		...
		private static void ApplyRotateTimeline(RotateTimeline rotateTimeline, Skeleton skeleton, float time, float alpha, bool setupPose, float[] timelinesRotation, int i, bool firstFrame)
		{
			...
			if (alpha == 1f)
			{
				rotateTimeline.Apply(skeleton, 0f, time, null, 1f, setupPose, false);
				return;
			}
		...
		private void SetCurrent(int index, TrackEntry current, bool interrupt)
		{
			...
			if (trackEntry != null)
			{
				...
				if (mixingFrom != null && trackEntry.mixDuration > 0f)
				{
					if (multipleMixing)
					{
					...
					{
						if (trackEntry.mixTime / trackEntry.mixDuration < 0.5f && mixingFrom.animation != EmptyAnimation)
						{
							current.mixingFrom = mixingFrom;
							mixingFrom.mixingFrom = trackEntry;
							mixingFrom.mixTime = trackEntry.mixDuration - trackEntry.mixTime;
							mixingFrom.mixDuration = trackEntry.mixDuration;
							trackEntry.mixingFrom = null;
							trackEntry = mixingFrom;
						}
						trackEntry.mixAlpha = 0f;
		...
		private void AnimationsChanged()
		{
			...
			for (i = 0; i < count; i++)
			{
				TrackEntry trackEntry3 = tracks.Items[i];
				if (trackEntry3 != null && trackEntry3.mixingFrom != null)
				{
					num = i;
					break;
				}
			...
			{
				TrackEntry trackEntry4 = tracks.Items[i];
				if (trackEntry4 != null)
				{
					ExposedList<Timeline> timelines = trackEntry4.animation.timelines;
					Timeline[] items = timelines.Items;
					int j = 0;
					for (int count2 = timelines.Count; j < count2; j++)
					{
						hashSet.Add(items[j].PropertyId);
					}
					for (trackEntry4 = trackEntry4.mixingFrom; trackEntry4 != null; trackEntry4 = trackEntry4.mixingFrom)
					{
						CheckTimelinesUsage(trackEntry4, trackEntry4.timelinesLast);
					}
				}
			}
		}
		...
		private void CheckTimelinesUsage(TrackEntry entry, ExposedList<bool> usageArray)
		{
			...
			ExposedList<Timeline> timelines = entry.animation.timelines;
			int count = timelines.Count;
			usageArray.EnsureCapacity(count);
			bool[] items = usageArray.Items;
			Timeline[] items2 = timelines.Items;
			for (int i = 0; i < count; i++)
			{
