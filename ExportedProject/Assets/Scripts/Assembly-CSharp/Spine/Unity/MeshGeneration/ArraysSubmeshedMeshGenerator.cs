...
{
	public class ArraysSubmeshedMeshGenerator : ArraysMeshGenerator, ISubmeshedMeshGenerator, IDisposable
	{
		private class SmartMesh : IDisposable
		{
		...
		public MeshAndMaterials GenerateMesh(SubmeshedMeshInstruction meshInstructions)
		{
			...
			bool flag2 = ArraysMeshGenerator.EnsureSize(vertexCount, ref meshVertices, ref meshUVs, ref meshColors32);
			...
			if (addTangents)
			{
				...
				for (int num = count; k < num; k++)
				{
					...
					ArraysMeshGenerator.SolveTangents2DTriangles(tempTanBuffer, submeshTriangleBuffer2.triangles, submeshTriangleBuffer2.triangleCount, meshVertices, meshUVs, vertexCount);
				}
