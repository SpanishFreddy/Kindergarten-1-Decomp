	...
	public class ArraysSubmeshSetMeshGenerator : ArraysMeshGenerator, ISubmeshSetMeshGenerator
	{
		...
		public MeshAndMaterials GenerateMesh(ExposedList<SubmeshInstruction> instructions, int startSubmesh, int endSubmesh, float scale = 1f)
		{
			...
			if (scale != 1f)
			{
				for (int n = 0; n < num; n++)
				{
					meshVertices[n].x *= scale;
					meshVertices[n].y *= scale;
				}
			}
			...
			if (flag3)
			{
				...
				{
					mesh.SetTriangles(submeshBuffers.Items[num2].triangles, num2);
				}
			...
			if (addTangents)
			{
				...
				int num3 = 0;
				for (int num4 = count; num3 < num4; num3++)
				{
					SubmeshTriangleBuffer submeshTriangleBuffer2 = submeshBuffers.Items[num3];
