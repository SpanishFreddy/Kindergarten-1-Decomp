	...
	public class IkConstraint : IConstraint, IUpdatable
	{
		...
		public static void Apply(Bone parent, Bone child, float targetX, float targetY, int bendDir, float alpha)
		{
			...
			else
			{
				...
				float num40 = (float)Math.Acos((0f - a) * num17 / (num22 - num23));
				num11 = a * (float)Math.Cos(num40) + num17;
				num12 = b * (float)Math.Sin(num40);
				...
				if (num24 <= (num33 + num37) / 2f)
				{
					num21 = num25 - (float)Math.Atan2(num35 * (float)bendDir, num34);
					num20 = num32 * (float)bendDir;
				}
				else
				{
					num21 = num25 - (float)Math.Atan2(num39 * (float)bendDir, num38);
					num20 = num36 * (float)bendDir;
				}
			}
			goto IL_0534;
			IL_0534:
			float num41 = (float)Math.Atan2(num7, ax2) * (float)num5;
			float arotation = parent.arotation;
			num21 = (num21 - num41) * (180f / (float)Math.PI) + (float)num4 - arotation;
			if (num21 > 180f)
			{
				num21 -= 360f;
			}
			...
			num20 = ((num20 + num41) * (180f / (float)Math.PI) - child.ashearX) * (float)num5 + (float)num6 - arotation;
			if (num20 > 180f)
			{
				num20 -= 360f;
			}
